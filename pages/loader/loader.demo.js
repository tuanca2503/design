class Loader {
  ...
  _ensureDom() {
    const build = () => {
      ...
      this._loader.classList.add("loader", `loader--${this.opts.style}`);
  ...
  _appendProgress() {
    ...
    this._text.className = "loader-text";
    this._bar = document.createElement("div");
    this._bar.className = "loader-bar";
    box.appendChild(this._bar);
    box.appendChild(this._text);

    this._loader.appendChild(box);
  }
  _appendSpinner() {
    const box = document.createElement("div");
    box.className = "loader-box";
  ...
  _appendDots() {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      const row = Math.floor(i / 3); // hàng: 0,1,2
      const col = i % 3; // cột:  0,1,2
      span.style.animationDelay = (row + col) * 0.1 + "s";
